...
mtcars$transmission = factor(ifelse(mtcars$am == 0,
                             "Automatic", "Manual"))
mtcars$engine = factor(ifelse(mtcars$vs == 0, 
                       "V-shape", "Straight"))
...
SELECT 
  PERCENTILE_CONT(0.5) WITHIN GROUP
   (ORDER BY weightkg) OVER () AS median
FROM dbo.mtcars;
GO
...
SELECT SUM(POWER((hp - a), 3) / POWER(s, 3)) / MIN(c)
 AS skewness,
 (SUM(POWER((hp - a), 4) / POWER(s, 4)) / MIN(c) - 
 3.0 * (MIN(c)-1) * (MIN(c)-1) / (MIN(c)-2) / (MIN(c)-3))
 AS kurtosis
FROM acs;
GO
...
SELECT MIN(c) AS hpCount,
 ...
 ROUND(SUM(POWER((hp - a), 3) / POWER(s, 3)) 
           / MIN(c), 2) AS hpSkew,
 ROUND((SUM(POWER((hp - a), 4) / POWER(s, 4)) / MIN(c) - 
            3.0 * (MIN(c)-1) * (MIN(c)-1) 
			/ (MIN(c)-2) / (MIN(c)-3)), 2) AS hpKurt
FROM acs;
GO
...
SELECT engine,
 hp,
 AVG(hp) OVER (PARTITION BY engine) AS a,
 COUNT(*) OVER (PARTITION BY engine) AS c,
 STDEV(hp) OVER (PARTITION BY engine) AS s,
 PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY hp)
  OVER (PARTITION BY engine) AS m,
 PERCENTILE_CONT(0.75) WITHIN GROUP (ORDER BY hp) 
  OVER (PARTITION BY engine)-
 PERCENTILE_CONT(0.25) WITHIN GROUP (ORDER BY hp) 
  OVER (PARTITION BY engine) AS i
FROM dbo.mtcars
)
SELECT engine,
 MIN(c) AS hpCount,
